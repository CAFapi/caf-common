...
- US969005: Add support for getting secrets  
  ...
  - Both these retrieve a secret value from configurable sources:
    - Environment variables (direct value) - enabled via `CAF_READ_SECRETS_FROM_ENV` (defaults to `true`)
    - File content (path specified by environment variable with `_FILE` suffix) - enabled via `CAF_READ_SECRETS_FROM_FILE` (defaults to `false`)
